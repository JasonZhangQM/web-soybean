...
defineOptions({ name: 'EnergyCrudeChart' });

/**
 * EIA 原油库存变动（柱状图，正负值不同颜色）
 * value ≥ 0 蓝（库存增加），< 0 橙（库存减少）
 */
interface Props {
...
const latestRows = computed(() =>
  buildLatestRows<Api.Bds.EconomicIndicator>(props.dataMap, [
    { code: 'EIA_CRUDE_OIL_INVENTORY_CHANGE', name: '原油库存变动', color: '#3b82f6', unit: '万桶' }
  ])
);
...
function buildOption() {
  ...
  const arr = getSeries(props.dataMap, 'EIA_CRUDE_OIL_INVENTORY_CHANGE');
