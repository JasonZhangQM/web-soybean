...
/**
 * EIA 汽油库存变动（柱状图，正负值不同颜色）
 * value ≥ 0 青（库存增加），< 0 红（库存减少）
 */
interface Props {
...
function buildOption() {
  ...
  const arr = getSeries(props.dataMap, 'EIA_GASOLINE_INVENTORY_CHANGE');
    ...
    series: [
      {
        ...
        type: 'bar',
        barMaxWidth: 24,
        // 正负值不同颜色：≥0 青（库存增加），<0 红（库存减少）
        itemStyle: {
          color: (params: { value: number }) => (Number(params.value) >= 0 ? '#14b8a670' : '#dc262670')
        },
        data: values
      }
    ]
