...
defineOptions({ name: 'MfgPmiCompareChart' });

/**
 * ISM 制造业 PMI vs 标普全球制造业 PMI
 * 双折线 + 50 荣枯线，以 ISM_MFG_PMI 日期为主轴
 */
interface Props {
...
// 构建 ECharts 配置：ISM 制造业 + 标普全球制造业 双折线，50 荣枯线
function buildOption() {
  const { ink, muted, rule } = getThemeColors();
  const ismArr = getSeries(props.dataMap, 'ISM_MFG_PMI');
  const spArr = getSeries(props.dataMap, 'SP_GLOBAL_MFG_PMI');

  // 以 ISM 制造业日期为主轴，按 report_date 对齐标普全球制造业
