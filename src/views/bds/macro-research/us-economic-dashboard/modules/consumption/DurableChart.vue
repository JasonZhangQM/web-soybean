...
defineOptions({ name: 'ConsumptionDurableChart' });

/**
 * 耐用品订单环比（柱状图，正负值不同颜色）
 * ≥0 紫 #7c3aed70，<0 红 #dc262670
...
// 构建 ECharts 配置：耐用品订单环比柱状图，按值正负动态着色
function buildOption() {
  const { ink, muted, rule } = getThemeColors();
  const arr = getSeries(props.dataMap, 'DURABLE_GOODS_ORDERS_MOM');
    ...
    series: [
      {
        ...
        // 按值正负动态着色：≥0 紫，<0 红
        itemStyle: {
          color: (params: any) => (Number(params.value) >= 0 ? '#7c3aed70' : '#dc262670')
        },
        data: values
      }
    ]
