...
import { fetchSymbolOptions } from '@/service/api';
import { trimSearchParams } from '@/utils/common';

defineOptions({ name: 'IrsSymbolOptionsPage' });

const loading = ref(false);
const tableData = ref<Api.Irs.SymbolOption[]>([]);
const total = ref(0);
...
async function fetchData() {
  ...
  try {
    ...
    const { data, error } = await fetchSymbolOptions({
      ...rest,
...
function handleReset() {
  searchParams.underlying_symbol = undefined;
  searchParams.underlying_name = undefined;
  searchParams.price_strike = undefined;
  searchParams.days_left = undefined;
  fetchData();
}
